  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      ...
      <div className="mb-10 flex items-start gap-5">
        {/* Avatar */}
        <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[#E8590C] to-[#16A34A] text-2xl font-bold text-white">
          {profile.name.charAt(0).toUpperCase()}
        </div>

        <div>
          <h1 className="text-2xl font-extrabold text-[#1A1A1A]">{profile.name}</h1>
          <a
            href={`https://github.com/${profile.github}`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-1 inline-flex items-center gap-1.5 text-sm text-[#6B7280] hover:text-[#1A1A1A] transition-colors"
          >
      ...
      {/* Divider */}
      <div className="mb-8 border-b border-black/[0.08]" />
      ...
      {workflows.length > 0 ? (
        ...
        <div className="flex flex-col items-center py-16 text-center">
          ...
          <Link
            href="/"
            className="mt-3 text-sm text-[#E8590C] hover:underline"
          >
