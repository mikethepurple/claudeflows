...
//
// Scans skills/*/workflow.json, reads prompt files for token estimates,
// and generates packages/web/lib/sample-data.ts.
//
...
const OUTPUT = path.join(ROOT, "packages/web/lib/sample-data.ts");

// ────────────────────────────────────────────────────────
// Hardcoded rich entries — these have detailedProcess, setupSteps,
// prerequisiteDetails, etc. that don't exist in workflow.json.
// Keep them as-is; they are the "showcase" skills.
// ────────────────────────────────────────────────────────

const ORIGINAL_SKILLS: Record<string, string> = {
  "venture-studio": `  {
    name: "venture-studio",
    ...
    steps: [
      {
        ...
        methodologyCount: 3,
        tokenEstimate: { inputTokens: 140000, outputTokens: 60000, totalTokens: 200000, confidence: "estimated" as const },
      },
      {
        name: "Opportunities",
        description: "Researches competitors and existing solutions, maps market gaps, identifies where current products fall short",
        methodologyCount: 2,
        tokenEstimate: { inputTokens: 105000, outputTokens: 45000, totalTokens: 150000, confidence: "estimated" as const },
      },
      {
        name: "Solutions",
        ...
        methodologyCount: 3,
        tokenEstimate: { inputTokens: 90000, outputTokens: 40000, totalTokens: 130000, confidence: "estimated" as const },
      },
      {
        name: "Business Model",
        description: "Designs pricing, revenue model, and unit economics based on competitor pricing and market expectations",
        methodologyCount: 2,
        tokenEstimate: { inputTokens: 80000, outputTokens: 35000, totalTokens: 115000, confidence: "estimated" as const },
      },
      {
        name: "Metrics",
        description: "Defines industry-calibrated KPIs: acquisition costs, conversion rates, retention targets, revenue milestones",
        methodologyCount: 2,
        tokenEstimate: { inputTokens: 40000, outputTokens: 20000, totalTokens: 60000, confidence: "estimated" as const },
      },
      {
        name: "Execution",
        description: "Builds your landing page, creates a GitHub repo, deploys to Vercel — you get a live site and all source files",
        methodologyCount: 4,
        tokenEstimate: { inputTokens: 65000, outputTokens: 55000, totalTokens: 120000, confidence: "estimated" as const },
      },
    ],
    mcp: {
      "brave-search": {
...
// Names of the original skills — these get their hardcoded entries, not generated from workflow.json
const ORIGINAL_SKILL_NAMES = new Set(Object.keys(ORIGINAL_SKILLS));

// ────────────────────────────────────────────────────────
// Read workflow.json files from skills/ and map to Workflow entries
// ────────────────────────────────────────────────────────
...
interface WorkflowJson {
  ...
  description: string;
  tagline?: string;
  author: { name: string; github: string; bio?: string };
  category: string;
  tags: string[];
  skills: { name: string; description: string }[];
  mcp: Record<string, { required: boolean; plainName?: string; plainDescription?: string; setupUrl?: string; setupTime?: string }>;
  repository: string;
  license?: string;
  difficulty?: string;
...
function estimateTokensFromPrompts(skillDir: string): { inputTokens: number; outputTokens: number; totalTokens: number } | null {
  const promptsDir = path.join(skillDir, "skills");
  ...
  const outputTokens = Math.round(inputTokens * 2);
  const totalTokens = inputTokens + outputTokens;

  return { inputTokens, outputTokens, totalTokens };
}

function buildEntryFromJson(wf: WorkflowJson, skillDir: string): string {
  const lines: string[] = [];
  lines.push(`  {`);
  lines.push(`    name: ${JSON.stringify(wf.name)},`);
  lines.push(`    displayName: ${JSON.stringify(wf.displayName)},`);
  if (wf.tagline) lines.push(`    tagline: ${JSON.stringify(wf.tagline)},`);
  lines.push(`    description: ${JSON.stringify(wf.description)},`);
  lines.push(`    author: ${JSON.stringify(wf.author)},`);
  lines.push(`    version: ${JSON.stringify(wf.version)},`);
  lines.push(`    category: ${JSON.stringify(wf.category)},`);
  lines.push(`    tags: ${JSON.stringify(wf.tags)},`);
  lines.push(`    installs: 0,`);
  lines.push(`    rating: 0,`);
  lines.push(`    skills: ${JSON.stringify(wf.skills)},`);
  lines.push(`    mcp: ${JSON.stringify(wf.mcp)},`);
  lines.push(`    repository: ${JSON.stringify(wf.repository)},`);
  if (wf.license) lines.push(`    license: ${JSON.stringify(wf.license)},`);
  if (wf.difficulty) lines.push(`    difficulty: ${JSON.stringify(wf.difficulty)} as const,`);
  if (wf.estimatedTime) lines.push(`    estimatedTime: ${JSON.stringify(wf.estimatedTime)},`);
  if (wf.setupTime) lines.push(`    setupTime: ${JSON.stringify(wf.setupTime)},`);
  if (wf.outputDescription) lines.push(`    outputDescription: ${JSON.stringify(wf.outputDescription)},`);
  if (wf.costInfo) lines.push(`    costInfo: ${JSON.stringify(wf.costInfo)},`);

  // Token estimate from prompt files
  const tokens = estimateTokensFromPrompts(skillDir);
  if (tokens) {
    lines.push(`    tokenEstimate: {`);
    lines.push(`      inputTokens: ${tokens.inputTokens},`);
    lines.push(`      outputTokens: ${tokens.outputTokens},`);
    lines.push(`      totalTokens: ${tokens.totalTokens},`);
    lines.push(`      confidence: "estimated" as const,`);
    lines.push(`      note: "Rough estimate based on prompt file sizes (chars / 4 for input, 2x for output).",`);
    lines.push(`    },`);
  }
...
function main() {
  ...
  const skillDirs = fs.readdirSync(SKILLS_DIR).filter((d) => {
    const wfPath = path.join(SKILLS_DIR, d, "workflow.json");
    return fs.existsSync(wfPath);
  });

  // Build entries for non-original skills
  const generatedEntries: { name: string; code: string }[] = [];

  for (const dir of skillDirs) {
    if (ORIGINAL_SKILL_NAMES.has(dir)) continue; // skip — use hardcoded version

    const wfPath = path.join(SKILLS_DIR, dir, "workflow.json");
    const raw = fs.readFileSync(wfPath, "utf-8");
    const wf: WorkflowJson = JSON.parse(raw);
    const entry = buildEntryFromJson(wf, path.join(SKILLS_DIR, dir));
    generatedEntries.push({ name: wf.name, code: entry });
  }

  // Sort generated entries alphabetically
  generatedEntries.sort((a, b) => a.name.localeCompare(b.name));

  // Assemble the file
  const originalOrder = ["venture-studio", "scout", "code-reviewer", "content-pipeline"];
  const originalBlocks = originalOrder.map((name) => ORIGINAL_SKILLS[name]);

  const allEntries = [
    ...originalBlocks,
    ...generatedEntries.map((e) => e.code),
  ];

  const output = `// Auto-generated by scripts/generate-sample-data.ts — do not edit manually.
...
`;
  ...
  console.log(`  Original skills: ${originalOrder.length}`);
  console.log(`  Generated from workflow.json: ${generatedEntries.length}`);
  console.log(`  Total: ${originalOrder.length + generatedEntries.length}`);
}
